...
function showInstallHelp(force = false) {
  ...
  const dismissed = localStorage.getItem("airsoftbrotherhoodnrw-install-help-dismissed") === "1";
...
function dismissInstallHelp() {
  localStorage.setItem("airsoftbrotherhoodnrw-install-help-dismissed", "1");
...
window.addEventListener("beforeinstallprompt", event => {
  ...
  window.setTimeout(() => showInstallHelp(false), 700);
});
...
window.addEventListener("load", () => {
  if (isIOSDevice() && !isStandaloneMode()) {
    window.setTimeout(() => showInstallHelp(false), 900);
  }
});
